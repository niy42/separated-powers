...
contract AgDAOCoin {
    ...
    event LawFinalized(string lawId, uint256 blockDelay);
    ...
    // Law finalization
    function finalizeLaw(string memory lawId, uint256 blockDelay) external onlyAdmin {
        laws[nextLawId] = Law(nextLawId, false, lawId);
        emit LawFinalized(lawId, blockDelay);
        nextLawId++;
    }
    ...
    function getAllProposals() public view returns(Proposals memory []){
        Return proposals;
    }
}
